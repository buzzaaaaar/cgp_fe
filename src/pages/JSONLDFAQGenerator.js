...
export default function JSONLDFAQGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('json_ld_faq', searchQuery);
      setFaqJson(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate JSON-LD FAQ');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!faqJson.trim()) {
      setError('No JSON-LD FAQ to save');
      return;
    }
    setSaveName(`JSON-LD FAQ - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'json_ld_faq', { faqJson, searchQuery });
      setSuccess('JSON-LD FAQ saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save JSON-LD FAQ');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save JSON-LD FAQ</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
