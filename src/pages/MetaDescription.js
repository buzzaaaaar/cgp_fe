...
export default function MetaDescription() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('meta_description', searchQuery);
      setDescription(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate meta description');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!description.trim()) {
      setError('No description to save');
      return;
    }
    setSaveName(`Meta Description - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'meta_description', { description, searchQuery });
      setSuccess('Meta description saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save meta description');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">SEO Meta Description</h1>
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Meta Description</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
