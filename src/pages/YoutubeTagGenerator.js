...
export default function YoutubeTagGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('youtube_tags', searchQuery);
  ...
  const handleSaveClick = () => {
    if (!tags.trim()) {
      setError('No tags to save');
      return;
    }
    setSaveName(`Youtube Tags - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'youtube_tags', { tags, searchQuery });
      setSuccess('Tags saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save tags');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        ...
        <div className="lg:w-1/3 w-full flex flex-col items-center">
          ...
          <div className="border border-gray-200 rounded-lg p-4 bg-white min-h-[150px] shadow-lg w-full max-w-sm mb-10">
            <div className="flex items-center mb-2">
              <img src={YoutubeHashtag} alt="Youtube Hashtag Generator" className="w-6 h-6 mr-2" />
              <h3 className="text-base font-semibold text-[#013024]">Youtube Hashtag Generator</h3>
            </div>
            <p className="text-sm text-gray-600 mb-2">
              Find the perfect hashtags to increase reach and engagement on your YouTube videos
            </p>
            <Link 
              to="/YoutubeHashtagGenerator" 
              className="text-sm text-[#013024] text-right block hover:underline hover:text-[#7FAF37]"
            >
              Learn more
            </Link>
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Tags</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
