...
export default function TopicalMapGenerator() {
  ...
  const [topicalMap, setTopicalMap] = useState('');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('topical_map', searchQuery);
      setTopicalMap(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate topical map');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!topicalMap.trim()) {
      setError('No topical map to save');
      return;
    }
    setSaveName(`Topical Map - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'topical_map', { topicalMap, searchQuery });
      setSuccess('Topical map saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save topical map');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Topical Map Generator</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="lg:w-2/3 w-full">
          ...
          <div className="mb-6">
            <TextField
              fullWidth
              label="Generated Topical Map"
              value={topicalMap}
              onChange={(e) => setTopicalMap(e.target.value)}
              margin="normal"
              multiline
              rows={8}
            />
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Topical Map</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
