...
export default function SubKeywordGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('sub_keywords', searchQuery);
      setKeywords(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate sub keywords');
    } finally {
  ...
  const handleSaveClick = () => {
    ...
    setSaveName(`Sub Keywords - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'sub_keywords', { keywords, searchQuery });
      setSuccess('Sub keywords saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save sub keywords');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Sub Keyword Generator</h1>
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Sub Keywords</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
