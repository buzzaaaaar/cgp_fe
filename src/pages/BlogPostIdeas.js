...
export default function BlogPostIdeas() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('blog_post_ideas', searchQuery);
      setIdeas(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate blog post ideas');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!ideas.trim()) {
      setError('No blog post ideas to save');
      return;
    }
    setSaveName(`Blog Post Ideas - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'blog_post_ideas', { ideas, searchQuery });
      setSuccess('Blog post ideas saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save blog post ideas');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Blog Post Ideas Generator</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="lg:w-2/3 w-full">
          ...
          <div className="mb-6">
            <TextField
              fullWidth
              label="Generated Blog Post Ideas"
              value={ideas}
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Blog Post Ideas</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
