...
import InstagramContentIcon from '../assets/instacontent.png';
...
export default function InstagramHashtagGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('instagram_hashtag', searchQuery);
      setHashtags(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate Instagram hashtags');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!hashtags.trim()) {
      setError('No Instagram hashtags to save');
      return;
    }
    setSaveName(`Instagram Hashtags - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'instagram_hashtag', { hashtags, searchQuery });
      setSuccess('Instagram hashtags saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save Instagram hashtags');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Instagram Hashtag Generator</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="lg:w-2/3 w-full">
          ...
          <div className="mb-6">
            <TextField
              fullWidth
              label="Generated Instagram Hashtags"
              value={hashtags}
        ...
        <div className="lg:w-1/3 w-full flex flex-col items-center">
          ...
          {/* Instagram Content Ideas Tool */}
          <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-white min-h-[150px] shadow-lg w-full max-w-sm">
            <div className="flex items-center mb-2">
              <img src={InstagramContentIcon} alt="Instagram Content Ideas Icon" className="w-6 h-6 mr-2" />
              <h3 className="text-base font-semibold text-[#013024]">Instagram Content Ideas</h3>
            </div>
            <p className="text-sm text-gray-600 mb-2">
              Get fresh and trending content ideas to engage your audience and grow your Instagram presence.
            </p>
            <Link 
              to="/InstagramContentIdeas" 
              className="text-sm text-[#013024] text-right block hover:underline hover:text-[#7FAF37]"
            >
              Learn more
            </Link>
          ...
          <Advertisement searchQuery={searchQuery} toolType="instagram_hashtag" />
        </div>
      </div>

      {/* Save Dialog */}
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Instagram Hashtags</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
