...
export default function TwitterPostGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('twitter_post', searchQuery);
      setTweet(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate Twitter post');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!tweet.trim()) {
      setError('No Twitter post to save');
      return;
    }
    setSaveName(`Twitter Post - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'twitter_post', { tweet, searchQuery });
      setSuccess('Twitter post saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save Twitter post');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Twitter Post Generator</h1>
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Twitter Post</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
