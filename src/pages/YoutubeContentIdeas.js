...
export default function YoutubeContentIdeas() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('youtube_content_ideas', searchQuery);
  ...
  const handleSaveClick = () => {
    ...
    setSaveName(`Youtube Content Ideas - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'youtube_content_ideas', { ideas, searchQuery });
        ...
        <div className="lg:w-1/3 w-full flex flex-col items-center">
          ...
          <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-white min-h-[150px] shadow-lg w-full max-w-sm">
            <div className="flex items-center mb-2">
              <img src={YoutubeTag} alt="Youtube Tag Generator" className="w-6 h-6 mr-2" />
              <h3 className="text-base font-semibold text-[#013024]">Youtube Tag Generator</h3>
            </div>
            <p className="text-sm text-gray-600 mb-2">
              Generate optimized tags for your YouTube videos to improve visibility and reach
            </p>
            <Link 
