...
import InstagramHashtagIcon from '../assets/instahashtag.png';
...
export default function InstagramCaptionGenerator() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('instagram_caption', searchQuery);
      setCaption(response.content);
    } catch (error) {
      setError(error.message || 'Failed to generate Instagram caption');
    } finally {
  ...
  const handleSaveClick = () => {
    if (!caption.trim()) {
      setError('No Instagram caption to save');
      return;
    }
    setSaveName(`Instagram Caption - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'instagram_caption', { caption, searchQuery });
      setSuccess('Instagram caption saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save Instagram caption');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Instagram Caption Generator</h1>
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <div className="lg:w-2/3 w-full">
          ...
          <div className="mb-6">
            <TextField
              fullWidth
              label="Generated Instagram Caption"
              value={caption}
        ...
        <div className="lg:w-1/3 w-full flex flex-col items-center">
          ...
          <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-white min-h-[150px] shadow-lg w-full max-w-sm">
            <div className="flex items-center mb-2">
              <img src={InstagramHashtagIcon} alt="Instagram Hashtag Generator Icon" className="w-6 h-6 mr-2" />
              <h3 className="text-base font-semibold text-[#013024]">Instagram Hashtag Generator</h3>
            </div>
            <p className="text-sm text-gray-600 mb-2">
              Generate relevant hashtags for your Instagram posts to increase visibility and reach.
            </p>
            <Link 
              to="/InstagramHashtagGenerator" 
              className="text-sm text-[#013024] text-right block hover:underline hover:text-[#7FAF37]"
            >
              Learn more
            </Link>
          ...
          <Advertisement searchQuery={searchQuery} toolType="instagram_caption" />
        </div>
      </div>

      {/* Save Dialog */}
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Instagram Caption</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
