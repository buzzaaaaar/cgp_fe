...
export default function FacebookContentIdeas() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await content.generate('facebook_content_ideas', searchQuery);
  ...
  const handleSaveClick = () => {
    if (!ideas.trim()) {
      setError('No content ideas to save');
      return;
    }
    setSaveName(`Facebook Content Ideas - ${new Date().toLocaleDateString()}`);
  ...
  const handleSave = async () => {
    ...
    try {
      await content.save(saveName, 'facebook_content_ideas', { ideas, searchQuery });
      setSuccess('Content ideas saved successfully');
      setIsSavePopupOpen(false);
    } catch (error) {
      setError(error.message || 'Failed to save content ideas');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white font-hanken px-4 sm:px-8 py-6">
      ...
      <div className="mb-6">
        <h1 className="text-5xl font-extrabold text-[#7FAF37] mb-4">Facebook Content Ideas</h1>
          ...
          {/* Facebook Hashtag Generator */}
           <div className="border border-gray-200 rounded-lg p-4 mb-6 bg-white min-h-[150px] shadow-lg w-full max-w-sm">
            <div className="flex items-center mb-2">
              <img src={FBHashtag} alt="Facebook Hashtag Generator" className="w-6 h-6 mr-2" />
              <h3 className="text-base font-semibold text-[#013024]">Facebook Hashtag Generator</h3>
            </div>
            <p className="text-sm text-gray-600 mb-2">
              Find the perfect hashtags to increase reach and engagement on your Facebook posts
            </p>
            <Link 
              to="/FacebookHashtagGenerator" 
      ...
      <Dialog open={isSavePopupOpen} onClose={handleClosePopup}>
        <DialogTitle>Save Content Ideas</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
